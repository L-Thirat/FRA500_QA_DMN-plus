...
def init_babi(fname):
    ...
    task = None
    print(fname)
    f = codecs.open(fname, 'r',encoding='utf8')#todo fuse
    ...
    # for i, line in enumerate(open(fname)):
        ...
        line = line.strip()
        line = line.replace('.', ' . ')
        line = line[line.find(' ')+1:] #line.find(' ')+1 = int(position)
        # if not a question
        if line.find('?') == -1:
            task["C"] += line
            id_map[id] = counter #number in front of sentence
            counter += 1
            
        else:
            idx = line.find('?')
            tmp = line[idx+1:].split('\t') #after question
            task["Q"] = line[:idx]
            task["A"] = "X" #tmp[1].strip() #sentence of answer
            task["S"] = []
            # for num in tmp[2].split(): #list of number **only one number for single support-fact
            task["S"].append("NULLLL")#id_map[int(num.strip())])
            tasks.append(task.copy()) #zip all of above to lists
...
def get_babi_raw(id, test_id):
    ...
    babi_train_raw = init_babi(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data/en-10k/%s_train.txt' % babi_name)) #todo use en-10k
    babi_test_raw = init_babi(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data/en-10k/%s_self_test.txt' % test_id))
...
def process_input(data_raw, floatX, word2vec, vocab, ivocab, embed_size, split_sentences=False):
    map_2wordvec = codecs.open("map_word2vec.txt", 'w',encoding='utf8')
    questions = []
    inputs = []
    answers = []
    input_masks = []
    ...
    for x in data_raw:
        if split_sentences:
            ...
            # print(inp) #[[marry,move,to,bathroom],[...]] ... Q
        else:
            inp = x["C"].lower().split(' ') 
            inp = [w for w in inp if len(w) > 0]

        q = x["Q"].lower().split(' ') #x[Q] no "?" now
        ...
        questions.append(np.vstack(q_vector).astype(floatX)) #only one sentence
        # print(x["A"])
        # asd
        # todo move to write file
        # x["A"] = x["A"].replace(u"ที่",u"ไม่")
        # x["A"] = x["A"].replace(u"บางที",u"อาจจะ")
        answers_vec = process_word(word = x["A"],
                                        word2vec = word2vec,
                                        vocab = vocab,
                                        ivocab = ivocab,
                                        word_vector_size = embed_size,
        ...
        if answers_vec not in answers:
            map_2wordvec.write(x["A"])
            map_2wordvec.write(",")
            map_2wordvec.write(str(answers_vec))
            map_2wordvec.write("\n")
        answers.append(answers_vec)
    ...
    # print(input_masks,'\n',len(input_masks))
    # print("__InpMask__")
    #input_masks used when split_sentences==False
    # asd
...
def load_babi(config, split_sentences=False):
    ...
    # print(config.word2vec_init) == FALSE
    # asd
    ...
    process_word(word = "<eos>", 
                ...
                word_vector_size = config.embed_size, 
                to_return = "index")
    # print(vocab) #use process word for word2vec by update vocab dict {word:index}
    # print(ivocab)#{index:word}
    ...
    # print(train_data[0][0])#for first [CQAS]
    # asd
    # print(config.word2vec_init)
    # asd
    if config.word2vec_init:
        assert config.embed_size == 100
        ...
        # print(config.embedding_init,len(ivocab),config.embed_size)
        # print(word_embedding)
        # print(len(word_embedding),len(word_embedding[0])) == (len(ivocab),config.embed_size)
        # asd
    ...
    if split_sentences:
        ...
        # print(input_lens[3], sen_lens[0], max_sen_len) #number of ref.sentence of C until Q, sen_lens[x] = number of word in sentence[x], max sen len
        # asd
    else:
    ...
    # print(q_lens) #number of word in Q sentence
    # asd

    max_q_len = np.max(q_lens)
    max_input_len = min(np.max(input_lens), config.max_allowed_inputs) #fix max len of input
